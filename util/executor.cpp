...
void executor::run_all( )
{
    ...
    const int nTests = collection::instance().get_test_count( );
    ...
    for ( int i = 0; i < nTests; i++ ) 
    {
        ...
        test_base * test = collection::instance().get_test( i );
        ...
        // scope for the logger
        {
            ...
            printer::instance( ).write( testInfo );   
            ...
            {
                logger.fail( "Exception thrown and not caught by test case!" );
            }

            // write the test transcript
            printer::instance( ).write( logger.get_info() );   
