...
public:
    ...
    virtual void run( util::logger &log ) override
    {
        try
        {
            ...
            */
            auto extensionSupported = device.has_extension( cl::sycl::string_class( "cl_khr_fp64" ) );
            ...
            */
            {
                if ( device.has_extension( "cl_ext_device_fission" ) )
                {
                    ...
                    if ( typeid( subDevices ) != typeid(cl::sycl::vector_class<cl::sycl::device>))
                    {
                        FAIL( log, "create_sub_devices_equally() does not return vector_class<device>" );
                    }
                }
            }
            ...
            */
            {
                if ( device.has_extension( "cl_ext_device_fission" ) )
                {
                    ...
                    auto subDevices = device.create_sub_devices_by_count( devicePartitionCounts );
                    if ( typeid( subDevices ) != typeid(cl::sycl::vector_class<cl::sycl::device>))
                    {
                        FAIL( log, "create_sub_devices_by_count() does not return vector_class<device>" );
                    }
                }
            }

            /** check create_sub_devices_by_affinity(vector_size<affinity_domain>) method
            */
            {
                if ( device.has_extension( "cl_ext_device_fission" ) )
                {
                    auto subDevices = device.create_sub_devices_by_count( cl::sycl::info::cl_device_affinity_domain_next_partitionable );
                    if ( typeid( subDevices ) != typeid(cl::sycl::vector_class<cl::sycl::device>))
                    {
                        FAIL( log, "create_sub_devices_by_affinity() does not return vector_class<device>" );
                    }
                }
            }
            ...
            */
            ...
            {
                FAIL( log, "get_devices() does not return cl::sycl::vector<cl::sycl::device>" );
            }
        }
