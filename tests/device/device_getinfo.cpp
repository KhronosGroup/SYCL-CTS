        ...
        */
        virtual void run(util::logger & log)
        {
            try
            {
                ...
                info_uint =
                    dev.get_info<CL_DEVICE_GLOBAL_MEM_CACHELINE_SIZE>();
                info_long = dev.get_info<CL_DEVICE_GLOBAL_MEM_SIZE>();
                // TODO: Next is optional - not even in header?
                ...
                info_vect_size_t =
                    dev.get_info<CL_DEVICE_MAX_WORK_ITEM_SIZES>();
                info_uint = dev.get_info<CL_DEVICE_MAX_WRITE_IMAGE_ARGS>();
                info_uint = dev.get_info<CL_DEVICE_MEM_BASE_ADDR_ALIGN>();
#if FULL_TEST_ENABLED //TODO:
                info_uint = dev.get_info<CL_DEVICE_MIN_DATA_TYPE_ALIGN_SIZE>();
#endif
                info_str  = dev.get_info<CL_DEVICE_NAME>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_CHAR>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_SHORT>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_INT>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_LONG>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_FLOAT>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE>();
                info_uint =
                    dev.get_info<CL_DEVICE_NATIVE_VECTOR_WIDTH_HALF>();
                info_str  =
                    dev.get_info<CL_DEVICE_OPENCL_C_VERSION>();
                info_dvid =
                    dev.get_info<CL_DEVICE_PARENT_DEVICE>();
                info_uint =
                    dev.get_info<CL_DEVICE_PARTITION_MAX_SUB_DEVICES>();

                info_vect_dpp =
                    dev.get_info<CL_DEVICE_PARTITION_PROPERTIES>();
                aff_domain =
                    dev.get_info<CL_DEVICE_PARTITION_AFFINITY_DOMAIN>();
                info_vect_dpp =
                    dev.get_info<CL_DEVICE_PARTITION_TYPE>();
                info_plid =
                    dev.get_info<CL_DEVICE_PLATFORM>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_CHAR>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_SHORT>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_INT>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_LONG>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_FLOAT>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE>();
                info_uint =
                    dev.get_info<CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF>();

                info_size_t = dev.get_info<CL_DEVICE_PRINTF_BUFFER_SIZE>();
                info_bool =
                    dev.get_info<CL_DEVICE_PREFERRED_INTEROP_USER_SYNC>();
                info_str  = dev.get_info<CL_DEVICE_PROFILE>();
                info_size_t =
                    dev.get_info<CL_DEVICE_PROFILING_TIMER_RESOLUTION>();
