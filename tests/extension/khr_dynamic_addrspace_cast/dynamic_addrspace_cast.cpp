...
namespace dynamic_addrspace_cast::tests {

TEST_CASE(
    "the dynamic addrspace cast extension defines the "
    "SYCL_KHR_DYNAMIC_ADDRSPACE_CAST macro",
    "[khr_dynamic_addrspace_cast]") {
#ifndef SYCL_KHR_DYNAMIC_ADDRSPACE_CAST
  static_assert(false, "SYCL_KHR_DYNAMIC_ADDRSPACE_CAST is not defined");
#endif
}

/*
 * This function checks the dynamic address space cast functionality by creating
 * a global buffer with the specified dimensions and casting a raw pointer to
 * that buffer to a global address space pointer. It verifies that the casted
 * pointer matches the original raw pointer. It also checks that casting to
 * local and private address space pointers results in null pointers.
 *
...
static void check_global_ptr_cast() {
  ...
  q.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(range, [=](sycl::item<dimensions> item) {
      ...
      auto global_ptr = sycl::khr::dynamic_addrspace_cast<
          sycl::access::address_space::global_space>(raw_global_ptr);

      // Cast the raw pointer to a local address space pointer
      auto local_ptr = sycl::khr::dynamic_addrspace_cast<
          sycl::access::address_space::local_space>(raw_global_ptr);

      // Cast the raw pointer to a private address space pointer
      auto private_ptr = sycl::khr::dynamic_addrspace_cast<
          sycl::access::address_space::private_space>(raw_global_ptr);
      ...
      success &= (local_ptr.get_raw() == nullptr);
      // Check if the private pointer is null
      success &= (private_ptr.get_raw() == nullptr);
...
TEST_CASE(
    "dynamic_addrspace_cast works on pointers in address_space::global_space",
    "[khr_dynamic_addrspace_cast]") {
...
/*
 * This function checks the dynamic address space cast functionality by creating
 * a local accessor with the specified dimensions and casting a raw pointer to
 * that accessor to a local address space pointer. It verifies that the casted
 * pointer matches the original raw pointer. It also checks that casting to
 * global and private address space pointers results in null pointers.
 *
 * @tparam Dims The dimensions of the test kernel.
 */
template <typename T, std::size_t... Dims>
  ...
  q.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(
        ...
        [=](sycl::nd_item<dimensions> item) {
          ...
          T raw_local_ptr{
              local_acc.template get_multi_ptr<sycl::access::decorated::no>()};

          // Cast the raw pointer to a global address space pointer
          auto global_ptr = sycl::khr::dynamic_addrspace_cast<
              sycl::access::address_space::global_space>(raw_local_ptr);
          ...
          // Cast the raw pointer to a private address space pointer
          auto private_ptr = sycl::khr::dynamic_addrspace_cast<
              sycl::access::address_space::private_space>(raw_local_ptr);

          // Check if the casted pointer matches the original raw pointer
          bool success = (sycl::bit_cast<std::size_t>(raw_local_ptr) ==
                          sycl::bit_cast<std::size_t>(local_ptr.get_raw()));
          // Check if the global pointer is null
          success &= (global_ptr.get_raw() == nullptr);
          // Check if the private pointer is null
          success &= (private_ptr.get_raw() == nullptr);
...
TEST_CASE(
    "dynamic_addrspace_cast works on pointers in address_space::local_space",
    "[khr_dynamic_addrspace_cast]") {
...
/*
 * This function checks the dynamic address space cast functionality by creating
 * a private variable and casting a raw pointer to that variable to a private
 * address space pointer. It verifies that the casted pointer matches the
 * original raw pointer. It also checks that casting to global and local
 * address space pointers results in null pointers.
 *
 * @tparam Dims The dimensions of the test kernel.
 */
template <typename T, std::size_t... Dims>
static void check_private_ptr_cast() {
  ...
  q.submit([&](sycl::handler& cgh) {
    ...
    cgh.parallel_for(range, [=](sycl::item<dimensions> item) {
      ...
      // Cast the raw pointer to a global address space pointer
      auto global_ptr = sycl::khr::dynamic_addrspace_cast<
          sycl::access::address_space::global_space>(raw_private_ptr);

      // Cast the raw pointer to a local address space pointer
      auto local_ptr = sycl::khr::dynamic_addrspace_cast<
          sycl::access::address_space::local_space>(raw_private_ptr);
...
TEST_CASE(
    "dynamic_addrspace_cast works on pointers in address_space::private_space",
    "[khr_dynamic_addrspace_cast]") {
