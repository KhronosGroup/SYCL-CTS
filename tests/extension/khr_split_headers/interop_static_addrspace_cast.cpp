...
// The sycl_khr_static_addrspace_cast extension states that when an
...
// <sycl/khr/static_addrspace_cast.hpp>, and the feature test macro is
...
#if defined(SYCL_KHR_SPLIT_HEADERS) && defined(SYCL_KHR_STATIC_ADDRSPACE_CAST)
...
TEST_CASE(
    "the static_addrspace_cast API is provided through "
    "<sycl/khr/static_addrspace_cast.hpp> when split_headers is implemented",
    "[khr_split_headers][static_addrspace_cast]") {
  // khr::static_addrspace_cast produces a non-decorated multi_ptr in the
  // requested address space.
  using return_t = decltype(sycl::khr::static_addrspace_cast<
...
TEST_CASE(
    "the static_addrspace_cast feature test macro is provided through "
    "<sycl/khr/split_headers/version.hpp>",
    "[khr_split_headers][static_addrspace_cast]") {
  STATIC_REQUIRE(SYCL_KHR_STATIC_ADDRSPACE_CAST >= 1);
}
