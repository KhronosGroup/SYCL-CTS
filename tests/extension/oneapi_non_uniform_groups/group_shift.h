    ...
    for (size_t test_case = 0;
      ...
      std::valarray<bool> res(false, test_matrix * work_group_size);
      {
        ...
        queue.submit([&](sycl::handler& cgh) {
          ...
          cgh.parallel_for<shift_non_uniform_group_kernel<
            ...
            if (!NonUniformGroupHelper<GroupT>::should_participate(sub_group,
                                                                   test_case)) {
              res_acc[0 * work_group_size + item.get_local_linear_id()] = true;
              res_acc[1 * work_group_size + item.get_local_linear_id()] = true;
              res_acc[2 * work_group_size + item.get_local_linear_id()] = true;
              res_acc[3 * work_group_size + item.get_local_linear_id()] = true;
              return;
            }
            ...
            shifted_var = sycl::shift_group_left(non_uniform_group, local_var);
            ...
            shifted_var = sycl::shift_group_right(non_uniform_group, local_var);
