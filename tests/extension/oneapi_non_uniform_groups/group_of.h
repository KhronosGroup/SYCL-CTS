      ...
      for (size_t size : sizes) {
        ...
        bool res[test_matrix * test_cases] = {false};
        {
          ...
          queue.submit([&](sycl::handler& cgh) {
            ...
            sycl::nd_range<1> executionRange(work_group_range,
                                             work_group_range);
            ...
            cgh.parallel_for<joint_of_group_kernel<
              ...
              res_acc[0] = !sycl::joint_any_of(non_uniform_group, v_begin,
                                               v_end, none_true);
              res_acc[1] = sycl::joint_any_of(non_uniform_group, v_begin, v_end,
                                              one_true);
              res_acc[2] = sycl::joint_any_of(non_uniform_group, v_begin, v_end,
                                              some_true);
              res_acc[3] = sycl::joint_any_of(non_uniform_group, v_begin, v_end,
                                              all_true);
              ...
              static_assert(
                  ...
                  "Predicate pred) is wrong\n");
              res_acc[4] = !sycl::joint_all_of(non_uniform_group, v_begin,
                                               v_end, none_true);
              res_acc[5] = !sycl::joint_all_of(non_uniform_group, v_begin,
                                               v_end, one_true);
              res_acc[6] = !sycl::joint_all_of(non_uniform_group, v_begin,
                                               v_end, some_true);
              res_acc[7] = sycl::joint_all_of(non_uniform_group, v_begin, v_end,
                                              all_true);
              ...
              static_assert(
                  ...
                  "Predicate pred) is wrong\n");
              res_acc[8] = sycl::joint_none_of(non_uniform_group, v_begin,
                                               v_end, none_true);
              res_acc[9] = !sycl::joint_none_of(non_uniform_group, v_begin,
                                                v_end, one_true);
...
struct predicate_function_of_non_uniform_group_test {
  void operator()(const std::string& type_name) {
    ...
    for (size_t test_case = 0;
      ...
      // Initially fill the results array with 'true'. Each non-uniform group
      // test 'ands' with this to ensure every non-uniform group in the
      // work-group returns the correct result.
      std::fill(res, res + total_case_count, true);
      {
        ...
        queue.submit([&](sycl::handler& cgh) {
          ...
          cgh.parallel_for<predicate_function_of_non_uniform_group_kernel<
            ...
            // which to test our predicates. Note that this has a well-defined
            // set of values [1,2,...,N] where N is the non-uniform group size.
            // Note that the non-uniform group could also just be of size 1.
            T local_var(non_uniform_group.get_local_linear_id() + 1);

            // predicates
            // The variable is never 1 for any member of the non-uniform group
            auto none_true = [&](T i) { return i == 0; };
            // Exactly one member of the non-uniform group has value 1 (the
            // first)
            auto one_true = [&](T i) { return i == 1; };
            ...
            {
              static_assert(std::is_same_v<bool, decltype(sycl::any_of_group(
                                                     non_uniform_group,
                                                     local_var, none_true))>,
                            "Return type of any_of_group(GroupT g, bool pred) "
                            "is wrong\n");
              res_acc[0] &=
              ...
              static_assert(std::is_same_v<bool, decltype(sycl::all_of_group(
                                                     non_uniform_group,
                                                     local_var, none_true))>,
                            "Return type of all_of_group(GroupT g, bool pred) "
                            "is wrong\n");
              res_acc[4] &=
                  !sycl::all_of_group(non_uniform_group, local_var, none_true);
              // Note that 'one_true' returns true for the first item. Thus in
              // the case that the non-uniform group size is 1, check that all
              // items match; otherwise check that not all items match.
              res_acc[5] &=
