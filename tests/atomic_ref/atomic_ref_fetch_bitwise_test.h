...
#define SYCL_CTS_ATOMIC_REF_FETCH_BITWISE_TEST_H
...
class atomic_ref_fetch_bitwise_test
  ...
  void check_test_result_buffer(std::array<bool, 9>& result,
    ...
    {
      INFO(description + "\nError returned type for fetch_xor() (" +
           addr_space + " space)");
      CHECK(result[6]);
    }
    {
      INFO(description + "\nError returned type for fetch_or() (" + addr_space +
           " space)");
      CHECK(result[7]);
    }
    {
      INFO(description + "\nError returned type for fetch_and() (" +
           addr_space + " space)");
      CHECK(result[8]);
    }
  }
  ...
  void run_on_device(const std::string& type_name,
    ...
    std::string description = get_section_name(
        type_name, memory_order, memory_scope, address_space, memory_order_val,
        memory_scope_val,
    ...
    auto fetch_bitwise_test = [memory_order_val, memory_scope_val](
      ...
      auto ref_val_before_xor = a_r.fetch_xor(
          operand_val_for_xor, memory_order_val, memory_scope_val);

      result_acc[0] = ref_val_before_xor == val_before_op;
      ...
      auto ref_val_before_or =
          a_r.fetch_or(operand_val_for_or, memory_order_val, memory_scope_val);

      result_acc[2] = ref_val_before_or == val_before_op;
      ...
      auto ref_val_before_and = a_r.fetch_and(
          operand_val_for_and, memory_order_val, memory_scope_val);

      result_acc[4] = ref_val_before_and == val_before_op;
    ...
    if constexpr (base::address_space_is_not_local_space()) {
      ...
      this->queue_submit_global_scope(result, fetch_bitwise_test);
    ...
    if constexpr (base::address_space_is_not_global_space()) {
      std::array result{false, false, false, false, false,
                        false, false, false, false};
      this->queue_submit_local_scope(result, fetch_bitwise_test);
...
#endif  // SYCL_CTS_ATOMIC_REF_FETCH_BITWISE_TEST_H
