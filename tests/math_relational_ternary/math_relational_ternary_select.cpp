...
typename type_t::elem_t next_param() {
  return static_cast<typename type_t::elem_t>(rand());
}
...
bool verify_func(scl_t result, vec_t p1, vec_t p2, vec_t p3) {
  return result == -reference::TEST_FUNC(p1, p2, p3);
}
...
struct test_class {
  ...
  void execute(util::logger &log, cl::sycl::queue &sycl_queue) {
    ...
    cl::sycl::buffer<type_t, 1> buf_param_1(&(m_params[0])[0],
                                            cl::sycl::range<1>(buffer_size_k));
    cl::sycl::buffer<type_t, 1> buf_param_2(&(m_params[1])[0],
                                            cl::sycl::range<1>(buffer_size_k));
    cl::sycl::buffer<type_t, 1> buf_param_3(&(m_params[2])[0],
                                            cl::sycl::range<1>(buffer_size_k));
