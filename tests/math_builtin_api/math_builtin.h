...
struct privatePtrCheck {
  returnT res;
  ArgT resArg;
  privatePtrCheck(returnT res_t, ArgT resArg_t) : res(res_t), resArg(resArg_t) {}
};
...
template <>
struct base<cl::sycl::half> {
  using type = std::uint16_t;
};

template <typename T>
std::string printable(T value) {
  const auto representation = *reinterpret_cast<typename base<T>::type*>(&value);
...
typename std::enable_if<std::is_floating_point<T>::value ||
    ...
    verify(sycl_cts::util::logger &log, T value, resultRef<T> r,
           int accuracy) {
  ...
  if (!std::isnan(value) && !std::isnan(reference) &&
      !std::isinf(value) && !std::isinf(reference)) {
    if (accuracy < 0)
...
void check_function_multi_ptr_private(sycl_cts::util::logger &log, funT fun,
 resultRef<returnT> ref, argT ptrRef, int accuracy = 0) {
  cl::sycl::range<1> ndRng(1);
  returnT kernelResult;
  ...
  try {
    ...
    testQueue.submit([&](cl::sycl::handler &h) {
      auto resultPtr = buffer.template get_access<cl::sycl::access::mode::write>(h);
      auto resultPtrArg = bufferArg.template get_access<cl::sycl::access::mode::write>(h);
      h.single_task<kernel<N>>([=]() {
...
void check_function_multi_ptr_global(sycl_cts::util::logger &log, funT fun,
            argT arg, resultRef<returnT> ref, argT ptrRef, int accuracy = 0) {
  cl::sycl::range<1> ndRng(1);
  returnT kernelResult;
  auto testQueue = makeQueueOnce();
  try {
    ...
    testQueue.submit([&](cl::sycl::handler &h) {
      ...
      cl::sycl::accessor<argT, 1, cl::sycl::access::mode::read_write,
          cl::sycl::access::target::global_buffer> globalAccessor(ptrBuffer, h);
      h.single_task<kernel<N>>([=]() {
...
void check_function_multi_ptr_local(sycl_cts::util::logger &log, funT fun,
  ...
  try {
    ...
    testQueue.submit([&](cl::sycl::handler &h) {
      ...
      h.single_task<kernel<N>>([arg, localAccessor, resultPtr, resultPtrArg, fun]() {
        localAccessor[0] = arg;
