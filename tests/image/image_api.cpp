...
unsigned int get_channel_type_size( unsigned int idx )
{
    ...
    {
        if ( g_channel_size[itter].idx == idx )
            return g_channel_size[itter].size;
    }
...
unsigned int get_channel_count( unsigned int idx )
{
    ...
    {
        if ( g_channels[itter].idx == idx )
            return g_channels[itter].size;
    }
...
collection g_test_set[] = { { CL_R,
                              { CL_SNORM_INT8, CL_SNORM_INT16, CL_UNORM_INT8, CL_UNORM_INT16, CL_UNORM_SHORT_565, CL_UNORM_SHORT_555,
                                CL_UNORM_INT_101010, CL_SIGNED_INT8, CL_SIGNED_INT16, CL_SIGNED_INT32, CL_UNSIGNED_INT8, CL_UNSIGNED_INT16,
                            ...
                            { CL_RGx,
                              { CL_SNORM_INT8, CL_SNORM_INT16, CL_UNORM_INT8, CL_UNORM_INT16, CL_UNORM_SHORT_565, CL_UNORM_SHORT_555,
                                CL_UNORM_INT_101010, CL_SIGNED_INT8, CL_SIGNED_INT16, CL_SIGNED_INT32, CL_UNSIGNED_INT8, CL_UNSIGNED_INT16,
                                CL_UNSIGNED_INT32, CL_HALF_FLOAT, CL_FLOAT, 0 } },
                            { CL_ARGB, { CL_UNORM_INT8, CL_SNORM_INT8, CL_SIGNED_INT8, CL_UNSIGNED_INT8, 0 } },
                            { CL_BGRA, { CL_UNORM_INT8, CL_SNORM_INT8, CL_SIGNED_INT8, CL_UNSIGNED_INT8, 0 } },
                            { CL_INTENSITY, { CL_UNORM_INT8, CL_UNORM_INT16, CL_SNORM_INT8, CL_SNORM_INT16, CL_HALF_FLOAT, CL_FLOAT, 0 } },
                            { CL_LUMINANCE, { CL_UNORM_INT8, CL_UNORM_INT16, CL_SNORM_INT8, CL_SNORM_INT16, CL_HALF_FLOAT, CL_FLOAT, 0 } },
...
{
public:
    void operator()( util::logger& log, range<dims>& r, range<dims - 1>* p = nullptr )
    {
        size_t l_order_itter, l_type_itter;

        // for each chanell order
        for ( l_order_itter = 0; g_test_set[l_order_itter].m_order != 0; l_order_itter++ )
        {
            // get number of the chanells
            unsigned int l_channels_count = get_channel_count( g_test_set[l_order_itter].m_order );

            // for each chanell type
            for ( l_type_itter = 0; g_test_set[l_order_itter].m_type[l_type_itter]; l_type_itter++ )
            {
                unsigned int l_channels_type_size = get_channel_type_size( g_test_set[l_order_itter].m_type[l_type_itter] );

                util::UNIQUE_PTR<char[]> image_host( new char[size * l_channels_type_size * l_channels_count] );
                ...
                image<dims> img( (void*)image_host.get(), g_test_set[l_order_itter].m_order, g_test_set[l_order_itter].m_type[l_type_itter],
                                 r );

                if ( r.get( 0 ) != img.get_range()[0] || r.get( 1 ) != img.get_range()[1] || r.get( 2 ) != img.get_range()[2] )
                {
                ...
                if ( p )
                {
                    ...
                    {
                        image<dims> img( (void*)image_host.get(), g_test_set[l_order_itter].m_order,
