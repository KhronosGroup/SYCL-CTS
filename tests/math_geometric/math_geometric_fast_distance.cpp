...
#define TEST_NAME math_geometric_fast_distance
...
T distance(cl::sycl::vec<T, n> v1, cl::sycl::vec<T, n> v2) {
  ...
  for (int i = 0; i < n; ++i)
    total +=
        (::getElement(v2, i) -
         ::getElement(v1, i) * (::getElement(v2, i) - ::getElement(v1, i)));
  return sqrt(total);
}
...
bool verify_func(double &input, type_t &param_x, type_t &param_y) {
  float ulpTolerance = 8194.f;

  float res = distance(param_x, param_y);
  if (fabs(res - input) > ulpTolerance) return false;
  return true;
}
...
} /* namespace math_geometric_fast_distance__ */
