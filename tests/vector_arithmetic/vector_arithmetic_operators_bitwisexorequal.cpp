...
#define TEST_NAME vector_arithmetic_operators_bitwisexorequal
#define KERNEL_NAME cKernel_vector_arithmetic_operators_bitwisexorequal
#define ARITHMETIC_OPERATOR ^=
#define COMPOUND_OPERATION 1
#define NAMESPACE vector_arithmetic_operators_bitwisexorequal__
    ...
    try {
      ...
      queue l_queue(selector);
      {
        ...
        l_queue.submit([&](handler &cgh) {
          auto xptr =
              xbuf.template get_access<cl::sycl::access::mode::read>(cgh);
          auto yptr =
              ybuf.template get_access<cl::sycl::access::mode::read>(cgh);
          auto optr =
