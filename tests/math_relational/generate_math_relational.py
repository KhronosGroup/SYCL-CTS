...
def generate( template_name, template_map ):
    ...
    vector_sizes = [ "1", "2", "3", "4", "8", "16" ]

    
    for item in template_map:
        for num_elms in vector_sizes:
            generated_content = template_content
            
            # find the mangled spir name
            spir_name = mangle( item['$TEST_FUNC$'], "ff" )
            
            # replace strings
            generated_content = generated_content.replace("$TEST_FUNC$", item['$TEST_FUNC$'])
            generated_content = generated_content.replace("$NUM_ELMS$", num_elms)

            # write generated file
            generated_file_name = template_name + "_" + item['$TEST_FUNC$'] + "_" + num_elms + ".cpp"
...
def main( ):
    ...
    generate( "math_relational" , g_relational_map )
    return
