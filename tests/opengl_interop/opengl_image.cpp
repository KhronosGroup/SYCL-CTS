...
void perform( util::logger &log, context & sycl_ctx )
{
    //buffer
    {
...

        {
            ...
            {
                auto img_acc = img.template get_access<float, cl::sycl::access::mode::read_write>( cgh );
                auto my_range = nd_range<1>(
                            range<1>( 24 ),
                            range<1>( 1 )
                            );
                auto my_kernel = ( [=]( item<1> item ) {
                    img_acc[item.get_global(0)] = golden;
                });
                cgh.parallel_for<class TEST_NAME>(
                            my_range,
                            my_kernel
                            );
            });

            auto img_acc_host = img.template get_access<float, cl::sycl::access::mode::read, access::host_image>();
            for( int i = 0; i < 24, i++ )
            {
    ...
    for( int i = 0; targets_3d[i]!= 0; i++ )
    {
        ...
        glTexImage3D( targets_3d[i], 0, GL_RGB, 2, 2, 1, 0, GL_RGB, GL_FLOAT, pixels );
