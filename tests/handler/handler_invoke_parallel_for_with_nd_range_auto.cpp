...
TestConstants constants;
  ...
  check_api_call("parallel_for(nd_range, functor) with auto", queue,
                 [&](handler &cgh, accessor_t acc) {
  ...
  check_api_call("parallel_for(nd_range, lambda) with auto, no kernel name", queue,
                 [&](handler &cgh, accessor_t acc) {
  ...
  check_api_call("parallel_for(nd_range, functor) with auto, no kernel name", queue,
                 [&](handler &cgh, accessor_t acc) {
  ...
  check_api_call(
      ...
      [&](handler &cgh, accessor_t acc) {
        cgh.parallel_for<class parallel_for_nd_range_offset_auto_functor_kernel>(
            constants.offsetNdRange, parallel_for_nd_range_auto_functor(acc));
      },
      constants.offset[0], constants.offsetRange[0]);
#if SYCL_CTS_ENABLE_FEATURE_SET_FULL
  check_api_call(
      "parallel_for(nd_range, lambda) with auto and offset, no kernel name", queue,
      [&](handler &cgh, accessor_t acc) {
      ...
      constants.offset[0], constants.offsetRange[0]);
  check_api_call(
      "parallel_for(nd_range, functor) with auto and offset, no kernel name", queue,
      [&](handler &cgh, accessor_t acc) {
        cgh.parallel_for(constants.offsetNdRange, parallel_for_nd_range_auto_functor(acc));
      },
      constants.offset[0], constants.offsetRange[0]);
