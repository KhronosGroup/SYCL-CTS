...
TestConstants constants;
  ...
  check_api_call("parallel_for(range, functor) with auto", queue,
                 [&](handler &cgh, accessor_t acc) {
                   using functor =
                       parallel_for_range_auto_functor<use_offset::no>;
  ...
  check_api_call("parallel_for(range, functor) with auto, no kernel name", queue,
                 [&](handler &cgh, accessor_t acc) {
                   cgh.parallel_for(
  ...
  check_api_call(
      ...
      [&](handler &cgh, accessor_t acc) {
        cgh.parallel_for(constants.offsetRange, constants.offset, [=](auto item) {
          parallel_for_range_auto_functor<use_offset::yes> f(acc);
          f(item);
        });
      },
