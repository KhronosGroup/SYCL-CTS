      ...
      try
        {
...

            {
              ...
              cl::sycl::buffer<int4, 1> gr_range_buffer( gr_range_data, range<1> ( l_items_total ) );
                ...
                auto gl_id_ptr = gl_id_buffer.get_access<cl::sycl::access::mode::read_write>( cgh );
                auto gl_size_ptr = gl_size_buffer.get_access<cl::sycl::access::mode::read_write>( cgh );
                auto l_id_ptr = l_id_buffer.get_access<cl::sycl::access::mode::read_write>( cgh );
                auto l_size_ptr = l_size_buffer.get_access<cl::sycl::access::mode::read_write>( cgh );
                ...
                cgh.parallel_for_work_group<class hierarchical_id>( my_range, [=]( group<3> group ) {
                    ...
                    parallel_for_work_item( group, [&]( item<3> item_id ) {
                        ...
                        gl_size_ptr[gl_id_L] = int4( gl_size_0, gl_size_1, gl_size_2, gl_size_L );
                        l_id_ptr[gl_id_L] = int4( l_id_0, l_id_1, l_id_2, l_id_L );
                        l_size_ptr[gl_id_L] = int4( l_size_0, l_size_1, l_size_2, l_size_L );
                    });
            ...
            for ( int k = 0; k < g_items_3d; k++ ) {
                for ( int j = 0; j < g_items_2d; j++ ) {
                    for ( int i = 0; i < g_items_1d; i++ ) {
                        int linear_index = ( ( k * g_items_2d * g_items_1d ) + ( j * g_items_1d ) + i );
            ...
            for ( int k = 0; k < gr_range_1d; k++ ) {
                for ( int j = 0; j < gr_range_2d; j++ ) {
                    for ( int i = 0; i < gr_range_3d; i++ ) {
                        int linear_index = ( ( k * gr_range_2d * gr_range_1d ) + ( j * gr_range_1d ) + i );
