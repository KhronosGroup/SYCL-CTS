...
void check_dim(util::logger &log) {
  ...
  {
    sycl::buffer<sycl::int4, 1> localIdBuffer(localIdData,
                                              sycl::range<1>(gl_items_total));
    sycl::buffer<sycl::int4, 1> localSizeBuffer(localSizeData,
                                                sycl::range<1>(gl_items_total));
    sycl::buffer<sycl::int4, 1> globalIdBuffer(globalIdData,
                                               sycl::range<1>(gl_items_total));
    sycl::buffer<sycl::int4, 1> globalSizeBuffer(
        globalSizeData, sycl::range<1>(gl_items_total));
    sycl::buffer<sycl::int4, 1> groupIdBuffer(groupIdData,
                                              sycl::range<1>(gr_range_total));
    ...
    myQueue.submit([&](sycl::handler &cgh) {
      ...
      auto l_range = sycl_cts::util::get_cts_object::range<
          dim>::template get_fixed_size<l_items_total>(l_items_1d, l_items_2d);
      ...
      cgh.parallel_for_work_group<kernel<dim>>(
          gr_range, l_range, [=](sycl::group<dim> group) {
            ...
            groupRangePtr[groupIdL] =
                sycl::int4(groupRange0, groupRange1, groupRange2, groupRangeL);
            ...
            group.parallel_for_work_item([&](sycl::h_item<dim> itemID) {
              ...
              localIdPtr[globalIdL] =
                  sycl::int4(localId0, localId1, localId2, localIdL);
              localSizePtr[globalIdL] =
                  sycl::int4(localSize0, localSize1, localSize2, localSizeL);
              globalIdPtr[globalIdL] =
                  sycl::int4(globalId0, globalId1, globalId2, globalIdL);
              globalSizePtr[globalIdL] = sycl::int4(globalSize0, globalSize1,
                                                    globalSize2, globalSizeL);
            });
          });
    });
  }
  ...
  for (int k = 0; k < check_g_items_3d; k++) {
    for (int j = 0; j < check_g_items_2d; j++) {
      for (int i = 0; i < check_g_items_1d; i++) {
        ...
        CHECK_VALUE(log, static_cast<int>(globalIdData[gLinearIndex].x()), i,
                    gLinearIndex);
        ...
        if (dim > 1) {
          ...
          CHECK_VALUE(log, static_cast<int>(globalIdData[gLinearIndex].y()), j,
                      gLinearIndex);
        ...
        if (dim > 2) {
          ...
          CHECK_VALUE(log, static_cast<int>(globalIdData[gLinearIndex].z()), k,
                      gLinearIndex);
