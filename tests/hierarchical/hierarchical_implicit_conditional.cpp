...

             {
               ...
               queue.submit([&]( handler & cgh )
               {
                             ...
                             my_range, [=](group<3> group )
                 {
                   ...
                   // recommened behaviour for the hierarchical API as there is a data race
                   // on the itemIds accessor and there is no gaurentee which work item id
                   // will be taken, this test makes sure that the assigment is only being
                   // done once.
