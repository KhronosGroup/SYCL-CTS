...
/** test specifiers
 */
#define TEST_NAME math_ternary_smoothstep
#define WIMPY_MODE 1
#define MAX_ULPS 4.0f
#define HOST_FUNC smoothstep
#define REF_FUNC reference::smoothstep

namespace math_unary_smoothstep__ {
 ...
 */
uint32_t gEdgeCases[] = {
    0x7fc00000, 0xff800000, 0xff7fffff, 0xdf800001, 0xdf800000, 0xdf7fffff,
    0xdf000001, 0xdf000000, 0xdeffffff, 0xcf800001, 0xcf800000, 0xcf7fffff,
    0xcf000001, 0xcf000000, 0xceffffff, 0xc47a0000, 0xc2c80000, 0xc0800000,
    0xc0600000, 0xc0400000, 0xc0400001, 0xc0200000, 0xc03fffff, 0xc0000000,
    0xbfc00001, 0xbfc00000, 0xbfbfffff, 0xbf800001, 0xbf800000, 0xbf7fffff,
    0xbf000001, 0xbf000000, 0xbeffffff, 0xbe800001, 0xbe800000, 0xbe7fffff,
    0x80800001, 0x80800000, 0x807fffff, 0x800007ff, 0x8000007f, 0x80000007,
    0x80000006, 0x80000005, 0x80000004, 0x80000003, 0x80000002, 0x80000001,
    0x80000000, 0xffc00000, 0x7f800000, 0x7f7fffff, 0x5f800001, 0x5f800000,
    0x5f7fffff, 0x5f000001, 0x5f000000, 0x5effffff, 0x4f800001, 0x4f800000,
    0x4f7fffff, 0x4f000001, 0x4f000000, 0x4effffff, 0x447a0000, 0x42c80000,
    0x40800000, 0x40600000, 0x40400000, 0x40400001, 0x40200000, 0x403fffff,
    0x40000000, 0x3fc00001, 0x3fc00000, 0x3fbfffff, 0x3f800001, 0x3f800000,
    0x3f7fffff, 0x3f000001, 0x3f000000, 0x3effffff, 0x3e800001, 0x3e800000,
    0x3e7fffff, 0x00800001, 0x00800000, 0x007fffff, 0x000007ff, 0x0000007f,
    0x00000007, 0x00000006, 0x00000005, 0x00000004, 0x00000003, 0x00000002,
...
class test_kernel {
 public:
  ...
  test_kernel(t_write w_, t_read p1_, t_read p2_, t_read p3_)
      : m_out(w_), m_p1(p1_), m_p2(p2_), m_p3(p3_) {}
...
class test_class {
 protected:
  ...
  bool execute_chunk(util::logger &log, queue &a_queue,
    ...
    try {
      buffer<type_t, 1> l_buf_param1(m_param_1.get() + chunkStart,
                                     range<1>(chunkSize));
      buffer<type_t, 1> l_buf_param2(m_param_2.get() + chunkStart,
                                     range<1>(chunkSize));
      buffer<type_t, 1> l_buf_param3(m_param_3.get() + chunkStart,
                                     range<1>(chunkSize));
      ...
      a_queue.submit([&](handler &cgh) {
        ...
        auto acc_p2 =
            l_buf_param2.template get_access<cl::sycl::access::mode::read>(cgh);
        auto acc_p3 =
            l_buf_param3.template get_access<cl::sycl::access::mode::read>(cgh);
  ...
  /**
   */
  bool verify_scalar(util::logger &log, base_t vut, base_t p1, base_t p2,
                     base_t p3) {
  ...
  bool verify(util::logger &log) {
    ...
    for (uint32_t i = 0; i < nBufferSize; i++) {
      for (uint32_t j = 0; j < num_elms; j++) {
        ...
        if (!verify_scalar(log, res, p1, p2, p3)) return false;
      }
    }
