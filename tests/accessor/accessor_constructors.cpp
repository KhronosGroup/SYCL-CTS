    ...
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> &range )
    {
        ...
        queue.submit( [&]( cl::sycl::handler& handler )
        {
            ...
            */
            ...
            cl::sycl::accessor<T, dims, mode, target> subAccessor( buffer, offset, r, handler );
        } );
    }
};

template <typename T, int dims, int size, cl::sycl::access::mode mode>
class buffer_accessor_constructors<T, dims, size, mode, cl::sycl::access::target::host_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue & queue, cl::sycl::range<dims> & range )
    {
        ...
        */
        cl::sycl::range<dims> offset = range / 2;
        cl::sycl::range<dims> r = range / 2;
        cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_buffer> subAccessor( buffer, offset, r );
    }
};

template <typename T, int dims, int size, cl::sycl::access::mode mode>
class buffer_accessor_constructors<T, dims, size, mode, cl::sycl::access::target::local>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> &range )
    {
        queue.submit( [&]( cl::sycl::handler& handler )
        {
            ...
            */
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::local> accessor( range, handler );
        } );
    }
};
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class image_accessor_constructors<T, dims, size, mode, cl::sycl::access::target::host_image>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> &range )
    {
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> &range )
    {
        ...
        */
        buffer_accessor_constructors<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );

        /** check buffe accessor constructors for write
        */
        buffer_accessor_constructors<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );

        /** check buffe accessor constructors for read_write
        */
        buffer_accessor_constructors<T, dims, size, cl::sycl::access::mode::read_write, target> readWriteTests;
        readWriteTests( log, queue, range );

        /** check buffe accessor constructors for disccard_write
        */
        buffer_accessor_constructors<T, dims, size, cl::sycl::access::mode::discard_write, target> discardWriteTests;
        discardWriteTests( log, queue, range );

        /** check buffe accessor constructors for discard_read_write
        */
        buffer_accessor_constructors<T, dims, size, cl::sycl::access::mode::discard_read_write, target> discardReadWriteTests;
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> &range )
    {
        ...
        */
        image_accessor_constructors<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );

        /** check image accessor constructors for write
        */
        image_accessor_constructors<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );
    }
};
