...
template <int dims>
cl::sycl::vector_class<cl::sycl::id<dims>> create_id_list( cl::sycl::range<dims> &r );

template <>
cl::sycl::vector_class<cl::sycl::id<1>> create_id_list<1>( cl::sycl::range<1> &r )
{
...
template <>
cl::sycl::vector_class<cl::sycl::id<2>> create_id_list<2>( cl::sycl::range<2> &r )
{
...
template <>
cl::sycl::vector_class<cl::sycl::id<3>> create_id_list<3>( cl::sycl::range<3> &r )
{
...
struct make_accessor {
  ...
  cl::sycl::accessor<T, dims, mode, target> operator()
        (cl::sycl::buffer<T, dims> buf, cl::sycl::handler& hand, cl::sycl::range<dims> range) {
    return buf.template get_access<mode, target>(hand);
  }

  cl::sycl::accessor<T, dims, mode, target> operator()
        (cl::sycl::image<dims> img, cl::sycl::handler& hand, cl::sycl::range<dims> range) {
...
struct make_accessor<T, dims, size, mode, cl::sycl::access::target::local> {
  ...
  cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::local> operator()(cl::sycl::buffer<T, dims> b, cl::sycl::handler& handler, cl::sycl::range<dims> range) {
    return cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::local>(range, handler);
  }
};

/**** get_subscript */
template<typename AccessorT, int dims, typename std::enable_if<dims == 1>::type* = nullptr>
typename AccessorT::ElementType get_linear_subscript(AccessorT acc, size_t index) {
  return typename AccessorT::ElementType(acc[index]);
}

template<typename AccessorT, int dims, typename std::enable_if<dims == 2>::type* = nullptr>
typename AccessorT::ElementType get_linear_subscript(AccessorT acc, size_t index) {
  /* Empty instance, this is not expected to work */
  return typename AccessorT::ElementType();
}

template<typename AccessorT, int dims, typename std::enable_if<dims == 3>::type* = nullptr>
typename AccessorT::ElementType get_linear_subscript(AccessorT acc, size_t index) {
  /* Empty instance, this is not expected to work */
  return typename AccessorT::ElementType();
}
...
typename AccessorT::ElementType get_subscript(AccessorT acc, cl::sycl::id<1> index) {
  return typename AccessorT::ElementType (acc[index[0]]);
}

template<typename AccessorT> 
typename AccessorT::ElementType get_subscript(AccessorT acc, cl::sycl::id<2> index) {
  return typename AccessorT::ElementType (acc[index[0]][index[1]]);
}

template<typename AccessorT> 
typename AccessorT::ElementType get_subscript(AccessorT acc, cl::sycl::id<3> index) {
  return typename AccessorT::ElementType (acc[index[0]][index[1]][index[2]]);
}


/***** check_element_valid */
template<typename T1, typename T2, typename std::enable_if<std::is_scalar<T1>::value>::type* = nullptr>
bool check_element_valid(const T1& elem, const T2& correct) {
  return elem == (static_cast<T1>(correct));
};

template<typename T1, typename T2, typename 
              std::enable_if< (!std::is_scalar<T1>::value) >::type* = nullptr>
bool check_element_valid(const T1& elem, const T2& correct) {
...
void set_subscript(AccessorT& acc, cl::sycl::id<2> index, size_t linearID) {
  acc[index[0]][index[1]] = static_cast<typename AccessorT::ElementType>(linearID);
}

template<typename AccessorT>
void set_subscript(AccessorT& acc, cl::sycl::id<3> index, size_t linearID) {
  acc[index[0]][index[1]][index[2]] = static_cast<typename AccessorT::ElementType>(linearID);
}

template<typename AccessorT, typename T, int dims>
void set_subscript(AccessorT& acc, cl::sycl::id<1> index, cl::sycl::vec<T, dims> vec) {
  acc[index[0]] = vec;
}

template<typename AccessorT, typename T, int dims>
void set_subscript(AccessorT& acc, cl::sycl::id<2> index, cl::sycl::vec<T, dims> vec) {
  acc[index[0]][index[1]] = vec;
}

template<typename AccessorT, typename T, int dims>
void set_subscript(AccessorT& acc, cl::sycl::id<3> index, cl::sycl::vec<T, dims> vec) {
  acc[index[0]][index[1]][index[2]] = vec;
}

/** Errror Types
 * When functors are executed on a parallel for the output type is an accessor,
 * but for functors executed on the host the output type is just an integer pointer.
 */
typedef  cl::sycl::accessor<int, 1, cl::sycl::access::mode::write, 
                      cl::sycl::access::target::global_buffer> errorOutputKernel_t;
 ...
 */
template <typename T, int dim, int size, cl::sycl::access::mode mode, cl::sycl::access::target target, typename ErrorOutputT>
class accessor_reads {
  ...
  public:
    ...
    accessor_reads(cl::sycl::accessor<T, dim, mode, target> accessorA,
                   ...
                   ErrorOutputT errorAccessor,
                   cl::sycl::range<dim> r) 
      : accessorA_(accessorA), accessorB_(accessorB), errorAccessor_(errorAccessor), r_(r) { }
    ...
    void operator()( cl::sycl::id<dim> i )
    {
      ...
      */
      elem = get_linear_subscript<decltype(accessorA_), dim>(accessorA_, linearID);
 ...
 */
template <typename T, int dim, int size, cl::sycl::access::mode mode, cl::sycl::access::target target>
...
class image_accessor_writes_kernel {
  ...
  public:
    ...
    image_accessor_writes_kernel(cl::sycl::accessor<T, dim, mode, target> accessorA,
                   cl::sycl::accessor<T, dim, mode, target> accessorB,
                   cl::sycl::range<dim> r) 
      : accessorA_(accessorA), accessorB_(accessorB), r_(r) { }
    ...
    {
      size_t linearID = compute_linear_id(i, r_);
      typename T::element_type elem0 = static_cast<typename T::element_type>(linearID);
...
class image_accessor_reads_kernel {
  ...
  public:
    ...
    image_accessor_reads_kernel(cl::sycl::accessor<T, dim, mode, target> accessorA,
                   cl::sycl::accessor<T, dim, mode, target> accessorB,
                   ...
                   ErrorOutputT errorAccessor,
                   cl::sycl::range<dim> r) 
      : accessorA_(accessorA), accessorB_(accessorB), 
        accessorC_(accessorC), accessorD_(accessorD), 
        sampler_(sampler),
        errorAccessor_(errorAccessor), r_(r) { }
...
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        T data[size];
        ...
        {
            auto accessor = make_accessor<T, dims, size, mode, target>()(buffer, handler, range);
            ...
            */
            ...
            if ( typeid( accessorPointer ) != typeid( typename explicit_pointer<T, target>::type ) )
            {
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class buffer_accessor_other_apis<T, dims, size, mode, cl::sycl::access::target::host_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_buffer> accessor( buffer );
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        cl::sycl::image<dims> image( data, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class image_accessor_other_apis<T, dims, size, mode, cl::sycl::access::target::host_image>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessor( image );
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
...

        {
            ...
            queue.submit( [&]( cl::sycl::handler &handler )
            {
                auto accessorA = make_accessor<T, dims, size, mode, target>()( bufferA, handler, range );
                auto accessorB = make_accessor<T, dims, size, mode, target>()( bufferB, handler, range );

                handler.parallel_for( range, accessor_writes<T, dims, size, mode, target>(accessorA, accessorB, range));
           } );
        }
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class buffer_accessor_writes<T, dims, size, mode, cl::sycl::access::target::host_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
...

        {
            ...
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_buffer> accessorA( bufferA );
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_buffer> accessorB( bufferB );

            auto idList = create_id_list<dims>(range);
            for (cl::sycl::id<dims> id : idList)
            {
                 accessor_writes<T, dims, size, mode, cl::sycl::access::target::host_buffer>(accessorA, accessorB, range)(id);
            }
        }
    ...
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
...

        {
            ...
            queue.submit( [&]( cl::sycl::handler &handler )
            {
                auto accessorA = make_accessor<T, dims, size, mode, target>() (bufferA, handler, range);
                auto accessorB = make_accessor<T, dims, size, mode, target>() (bufferB, handler, range);
                cl::sycl::accessor<int, 1, cl::sycl::access::mode::write, cl::sycl::access::target::global_buffer> errorAccessor( errorBuffer, handler );

                handler.parallel_for( range, accessor_reads<T, dims, size, mode, target, errorOutputKernel_t>(accessorA, accessorB, errorAccessor, range));
            } );
        }
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class buffer_accessor_reads<T, dims, size, mode, cl::sycl::access::target::host_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
...

        {
            ...
            for (cl::sycl::id<dims> id : idList)
            {
                accessor_reads<T, dims, size, mode, cl::sycl::access::target::host_buffer, errorOutputHost_t>(accessorA, accessorB, errors, range)(id);
            }
        }
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        {
            cl::sycl::image<dims> imageA( dataA, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            cl::sycl::image<dims> imageB( dataB, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            ...
            queue.submit( [&]( cl::sycl::handler &handler )
            {
                ...
                handler.parallel_for(range, image_accessor_writes_kernel<T, dims, size, mode, target>(accessorA, accessorB, range));
            } );
        }
...
template <typename T, int dims, int size, cl::sycl::access::mode mode>
class image_accessor_writes<T, dims, size, mode, cl::sycl::access::target::host_image>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        
        {
            ...
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorA( imageA );
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorB( imageB );

            auto idList = create_id_list<dims>(range);
            for (cl::sycl::id<dims> id : idList)
            {
                image_accessor_writes_kernel<T, dims, size, mode, cl::sycl::access::target::host_image>(accessorA, accessorB, range)(id);
            }
        }
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        
        {
            cl::sycl::image<dims> imageA( dataA, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            cl::sycl::image<dims> imageB( dataB, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            cl::sycl::image<dims> imageC( dataA, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            cl::sycl::image<dims> imageD( dataB, cl::sycl::image_format::channel_order::RGBA, cl::sycl::image_format::channel_type::UNSIGNED_INT8, range );
            ...
            queue.submit( [&]( cl::sycl::handler &handler )
            {
                ...
                handler.parallel_for(range, image_accessor_reads_kernel<T, dims, size, mode, target, errorOutputKernel_t>(accessorA, accessorB, accessorC, accessorD, sampler, errorAccessor, range));
           } );
        }
        ...
        if (errors[3] != 0)
        {
            FAIL( log, "operator[int][int][int](sampler) did not assign to the correct index" );
        }
    }
};

template <typename T, int dims, int size, cl::sycl::access::mode mode>
class image_accessor_reads<T, dims, size, mode, cl::sycl::access::target::host_image>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        
        {
            ...
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorA( imageA );
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorB( imageB );
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorC( imageC );
            cl::sycl::accessor<T, dims, mode, cl::sycl::access::target::host_image> accessorD( imageD );
            ...
            for (cl::sycl::id<dims> id : idList)
            {
                image_accessor_reads_kernel<T, dims, size, mode, cl::sycl::access::target::host_image, errorOutputHost_t>(accessorA, accessorB, accessorC, accessorD, sampler, errors, range)(id);
            }
        }
...
class buffer_accessor_subscripts<T, dims, size, cl::sycl::access::mode::read, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        buffer_accessor_reads<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );
    }
};

template <typename T, int dims, int size, cl::sycl::access::target target>
class buffer_accessor_subscripts<T, dims, size, cl::sycl::access::mode::write, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        buffer_accessor_writes<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );
    }
};

template <typename T, int dims, int size, cl::sycl::access::target target>
class buffer_accessor_subscripts<T, dims, size, cl::sycl::access::mode::read_write, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor subscript operators for read
        */
        buffer_accessor_reads<T, dims, size, cl::sycl::access::mode::read_write, target> readTests;
        readTests( log, queue, range );

        /** check buffer accessor subscript operators for write
        */
        buffer_accessor_writes<T, dims, size, cl::sycl::access::mode::read_write, target> writeTests;
        writeTests( log, queue, range );
    }
};

template <typename T, int dims, int size, cl::sycl::access::target target>
class buffer_accessor_subscripts<T, dims, size, cl::sycl::access::mode::discard_write, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor subscript operators for write
        */
        buffer_accessor_writes<T, dims, size, cl::sycl::access::mode::discard_write, target> writeTests;
        writeTests( log, queue, range );
    }
};

template <typename T, int dims, int size, cl::sycl::access::target target>
class buffer_accessor_subscripts<T, dims, size, cl::sycl::access::mode::discard_read_write, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor subscript operators for read
        */
        buffer_accessor_reads<T, dims, size, cl::sycl::access::mode::discard_read_write, target> readTests;
        readTests( log, queue, range );

        /** check buffer accessor subscript operators for write
        */
        buffer_accessor_writes<T, dims, size, cl::sycl::access::mode::discard_read_write, target> writeTests;
        writeTests( log, queue, range );
    }
};
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        image_accessor_reads<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );

        /** check buffer accessor other apis
        */
        image_accessor_other_apis<T, dims, size, cl::sycl::access::mode::read, target> otherAPITests;
        otherAPITests( log, queue, range );
    }
};

template <typename T, int dims, int size, cl::sycl::access::target target>
class image_accessor_apis<T, dims, size, cl::sycl::access::mode::write, target>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        image_accessor_writes<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );

        /** check buffer accessor other apis
        */
        image_accessor_other_apis<T, dims, size, cl::sycl::access::mode::write, target> otherAPITests;
        otherAPITests( log, queue, range );
    }
};
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );

        /** check buffer accessor api for write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );

        /** check buffer accessor api for read_write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read_write, target> readWriteTests;
        readWriteTests( log, queue, range );

        /** check buffer accessor api for disccard_write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::discard_write, target> discardWriteTests;
        discardWriteTests( log, queue, range );

        /** check buffer accessor api for discard_read_write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::discard_read_write, target> discardReadWriteTests;
        discardReadWriteTests( log, queue, range );
    }
};

template <typename T, int dims, int size>
class buffer_accessor_modes<T, dims, size, cl::sycl::access::target::host_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor api for read
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read, cl::sycl::access::target::host_buffer> readTests;
        readTests( log, queue, range );

        /** check buffer accessor api for write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::write, cl::sycl::access::target::host_buffer> writeTests;
        writeTests( log, queue, range );

        /** check buffer accessor api for read_write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read_write, cl::sycl::access::target::host_buffer> readWriteTests;
        readWriteTests( log, queue, range );
    }
};

template <typename T, int dims, int size>
class buffer_accessor_modes<T, dims, size, cl::sycl::access::target::constant_buffer>
{
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor api for read
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read, cl::sycl::access::target::constant_buffer> readTests;
        readTests( log, queue, range );
    }
};
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        /** check buffer accessor api for read_write
        */
        buffer_accessor_apis<T, dims, size, cl::sycl::access::mode::read_write, cl::sycl::access::target::local> readTests;
        readTests( log, queue, range );
    }
};
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        image_accessor_apis<T, dims, size, cl::sycl::access::mode::read, target> readTests;
        readTests( log, queue, range );

        /** check image accessor api for write
        */
        image_accessor_apis<T, dims, size, cl::sycl::access::mode::write, target> writeTests;
        writeTests( log, queue, range );
    }
};
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        buffer_accessor_modes<T, dims, size, cl::sycl::access::target::global_buffer> globalBufferTests;
        globalBufferTests( log, queue, range );

        /** check buffer accessor api for constant_buffer
        */
        buffer_accessor_modes<T, dims, size, cl::sycl::access::target::constant_buffer> constantTests;
        constantTests( log, queue, range );

        /** check buffer accessor api for host_buffer
        */
        buffer_accessor_modes<T, dims, size, cl::sycl::access::target::host_buffer> hostBufferTests;
        hostBufferTests( log, queue, range );

        /** check buffer accessor api for local
        */
        buffer_accessor_modes<T, dims, size, cl::sycl::access::target::local> localTests;
...
public:
    void operator()( util::logger &log, cl::sycl::queue &queue, cl::sycl::range<dims> range )
    {
        ...
        */
        image_accessor_modes<T, dims, size, cl::sycl::access::target::image> imageTests;
        imageTests( log, queue, range );

        /** check image accessor api for host_image
        */
        image_accessor_modes<T, dims, size, cl::sycl::access::target::host_image> hostImageTests;
