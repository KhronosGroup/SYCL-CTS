...
namespace generic_accessor_placeholder_buffer_constructor_exceptions_fp16 {

DISABLED_FOR_TEST_CASE(hipSYCL, ComputeCpp)
("Generic sycl::accessor placeholder buffer constructor exceptions. fp16 type",
 "[accessor]")({
  ...
  if (queue.get_device().has(sycl::aspect::fp16)) {
    ...
    for_type_vectors_marray<
        run_generic_placeholder_buffer_constructor_exceptions_test, sycl::half>(
        "sycl::half");
#else
    run_generic_placeholder_buffer_constructor_exceptions_test<sycl::half>{}(
        "sycl::half");
