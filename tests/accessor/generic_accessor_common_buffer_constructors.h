...
//
//  Provides tests for generic sycl::accessor constructors with buffers
//
*******************************************************************************/
#ifndef SYCL_CTS_GENERIC_ACCESSOR_COMMON_BUFFER_CONSTRUCTORS_H
#define SYCL_CTS_GENERIC_ACCESSOR_COMMON_BUFFER_CONSTRUCTORS_H
#include "accessor_common.h"

#include "catch2/catch_test_macros.hpp"

namespace generic_accessor_common_buffer_constructors {
...
void test_common_buffer_constructors(const std::string& type_name,
  ...
  const auto offset = util::get_cts_object::id<buf_dims>::get(0, 0, 0);
  ...
  section_name =
      get_section_name<Dimension>(type_name, access_mode_name, target_name,
                                  "From zero-length buffer constructor");
  ...
  if constexpr (0 != Dimension) {
    ...
    SECTION(section_name) {
      auto get_acc_functor = [r, offset](
                                 sycl::buffer<DataT, buf_dims>& data_buf,
                                 sycl::handler& cgh) {
        return sycl::accessor<DataT, Dimension, AccessMode, Target>(
...
class run_tests_common_buffer_constructors {
...
class run_generic_common_buffer_constructors_test {
 public:
  void operator()(const std::string& type_name) {
    ...
    for_all_combinations<run_tests_common_buffer_constructors, T>(
    ...
    const auto read_only_acc_mode =
        value_pack<sycl::access_mode, sycl::access_mode::read>::generate_named(
            "access_mode::read");
    for_all_combinations<run_tests_common_buffer_constructors, const T>(
        read_only_acc_mode, targets, dimensions, actual_type_name);
  }
};
}  // namespace generic_accessor_common_buffer_constructors
#endif  // SYCL_CTS_GENERIC_ACCESSOR_COMMON_BUFFER_CONSTRUCTORS_H
