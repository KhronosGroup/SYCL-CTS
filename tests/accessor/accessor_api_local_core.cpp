  ...
  void run(util::logger &log) override {
    try {
      ...
      if (!std::is_same<std::int8_t, cl::sycl::cl_char>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::int8_t>(
            log, queue, "std::int8_t");
      }
#endif
#ifdef UINT8_MAX
      if (!std::is_same<std::uint8_t, cl::sycl::cl_uchar>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::uint8_t>(
            log, queue, "std::uint8_t");
      }
#endif
#ifdef INT16_MAX
      if (!std::is_same<std::int16_t, cl::sycl::cl_short>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::int16_t>(
            log, queue, "std::int16_t");
      }
#endif
#ifdef UINT16_MAX
      if (!std::is_same<std::uint16_t, cl::sycl::cl_ushort>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::uint16_t>(
            log, queue, "std::uint16_t");
      }
#endif
#ifdef INT32_MAX
      if (!std::is_same<std::int32_t, cl::sycl::cl_int>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::int32_t>(
            log, queue, "std::int32_t");
      }
#endif
#ifdef UINT32_MAX
      if (!std::is_same<std::uint32_t, cl::sycl::cl_uint>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::uint32_t>(
            log, queue, "std::uint32_t");
      }
#endif
#ifdef INT64_MAX
      if (!std::is_same<std::int64_t, cl::sycl::cl_long>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::int64_t>(
            log, queue, "std::int64_t");
      }
#endif
#ifdef UINT64_MAX
      if (!std::is_same<std::uint64_t, cl::sycl::cl_ulong>::value) {
        for_type_and_vectors<check_local_accessor_api_type, std::uint64_t>(
      ...
      for_all_types_and_vectors<check_local_accessor_api_type>(
           vector_types, log, queue);

      for_all_types<check_local_accessor_api_type>(
