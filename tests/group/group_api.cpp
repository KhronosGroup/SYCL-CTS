...
class test_helper {
 ...
 public:
  test_helper(sycl::range<dimensions> globalRange,
    ...
    for (size_t i = 0; i < NUM_RESULTS; i++) {
      ...
      for (auto& value : callResult.values) value = 0;
    }
  }
  ...
  void collect_group_indicies(sycl::queue& queue) {
    ...
    queue.submit([&](sycl::handler& cgh) {
      auto a_dev = buf.template get_access<sycl::access_mode::read_write>(cgh);

      cgh.parallel_for_work_group<test_kernel<dimensions>>(
          m_globalRange, m_localRange, [=](sycl::group<dimensions> my_group) {
            ...
            // get(int)
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::get_dims)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::local_range)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::local_range_dims)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::global_range)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::global_range_dims)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::group_range)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::group_range_dims)];
            ...
            {
              call_result_t& callResult = a_dev[getter::get_index(
                  groupLinearID, getter::method::subscript)];
              ...
              using expected_t = size_t;
              callResult.hasValidType =
                  std::is_same<expected_t, decltype(my_group[0])>::value;
            }
          });
    });
  }
 ...
 private:
  ...
  void validate_group_indicies(
    ...
    for (size_t groupID0 = 0; groupID0 < m_globalRange[0]; ++groupID0) {
      for (size_t groupID1 = 0; groupID1 < m_globalRange[1]; ++groupID1) {
        ...
        validate_group_indicies(log, groupLinearID,
                                std::array<size_t, 2>{groupID0, groupID1});
      }
    }
  }
  ...
  void check_indices(
      util::logger& log, size_t groupLinearId, getter::method getterMethod,
    ...
    const auto& callResult =
        m_callResults[getter::get_index(groupLinearId, getterMethod)];
    ...
    if (!callResult.hasValidType) {
      ...
      log.note("  -> group %d: %s", groupLinearId, getter::name(getterMethod));
    }
  }
 ...
 public:
  ...
  void run(util::logger& log) override {
    ...
    // Validate for each dimension possible
    {
      ...
      auto validator = test_helper<1>(sycl::range<1>(GROUP_RANGE_1D),
    ...
    }
    {
      ...
      auto validator =
          test_helper<2>(sycl::range<2>(GROUP_RANGE_1D, GROUP_RANGE_2D),
