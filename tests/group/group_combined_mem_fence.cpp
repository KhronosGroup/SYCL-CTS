...
//
//  Provide coverage for global_and_local mem_fence
//
*******************************************************************************/
...
#define TEST_NAME group_combined_mem_fence

namespace group_combined_mem_fence__ {
...
void test_mem_fence(util::logger &log, sycl::queue &queue) {
  ...
  const auto fenceCallFactory = make_fence_call_factory(
      [=](sycl::group<dim> item) { item.mem_fence(fenceSpace); },
      [=](sycl::group<dim> item) {
