...
class cKernel_mit {
 protected:
  typedef accessor<Vec, 1, access::mode::read,
 ...
 public:
  cKernel_mit(t_writeAccess out_, t_readAccess x_, t_readAccess y_,
              t_readAccess z_)
      : m_o(out_), m_x(x_), m_y(y_), m_z(z_) {}
    ...
    for (int i = 0; i < nBufferSize; i++) {
      ...
      for (int j = 0; j < nVectorElements; j++) {
        ...
        if (!pass) {
          ...
          log.note(
              TOSTRING(TEST_TYPE) "( 0x%llx, 0x%llx, 0x%llx ) returned 0x%llx",
