...
/*************************************************************************
//
//  This file was AUTOMATICALLY GENERATED via generate_integer_ternary.py
//
**************************************************************************/
/*************************************************************************
//
...
/** test specifiers
 */
#define TEST_NAME    math_integer_ternary_mad_hi
#define TEST_TYPE    mad_hi
#define ARG_TYPE     uint32_t

namespace math_integer_ternary_mad_hi__
{
...
{
protected:
    typedef accessor<Vec, 1, access::read,  cl::sycl::access::target::global_buffer> t_readAccess;
    ...
    t_readAccess  m_z; /* argument Z */

public:
    cKernel_mit( t_writeAccess out_, t_readAccess x_, t_readAccess y_, t_readAccess z_ )
        : m_o( out_ )
        , m_x( x_ )
        , m_y( y_ )
        , m_z( z_ )
...
protected:
    ...
    util::UNIQUE_PTR<Vec[]> m_zbuf; /* y_arguments */
...
public:
    ...
    void generate( util::logger &, MTdata &rng )
    {
        ...
        math::rand( rng, (uint8_t*)m_zbuf.get(), nBytes );
    }
    ...
    bool execute( util::logger &log )
    {
        ...
        try
        {
            ...
            buffer<Vec, 1> zbuf( m_zbuf.get(), range<1> ( nBufferSize ) );
            ...
            l_queue.submit( [&]( handler& cgh )
            {
                ...
                auto zptr = zbuf.template get_access<cl::sycl::access::mode::read >( cgh );

                /* instantiate the kernel */
                auto kern = cKernel_mit<Vec>( optr, xptr, yptr, zptr );
     ...
     */
    bool verify( Base result, Base x, Base y, Base z )
    {
        /* scalar results are 1 on pass and 0 on fail */
#if 0
        return result == reference::TEST_TYPE( x, y, z );
#else
        return true;
#endif
    }
    ...
    bool verify( util::logger &log )
    {
        ...
        for ( int i = 0; i < nBufferSize; i++ )
        {
            ...
            Base* ez_a = reinterpret_cast<Base*>( z+i );
            ...
            for ( int j = 0; j < nVectorElements; j++ )
            {
                ...
                pass = verify( eo, ex, ey, ez );

                if ( !pass )
                {
                    log.note( "fail at item %d element %d", i, j );
                    log.note( TOSTRING( TEST_TYPE ) "( 0x%llx, 0x%llx, 0x%llx ) returned 0x%llx",
                              (uint64_t)ex, (uint64_t)ey, (uint64_t)ez, (uint64_t)eo );
                    FAIL( log, "" );
                    return false;
                }
            }
    ...
    bool setup( util::logger & )
    {
        ...
        m_zbuf.reset( new Vec[nBufferSize] );
        assert( m_zbuf.get() != nullptr );
        memset( m_zbuf.get( ), 0, sizeof(Vec)* nBufferSize );
