...
#define SAME_TYPE( X, Y, Z ) { auto e = X; same_type<decltype(e),Y> v( Z ); }


typedef accessor<int32_t, 1, cl::sycl::access::mode::read_write, cl::sycl::access::target::global_buffer> pass_t;
...
public:
    ...
    type_check_kernel_v16( pass_t p )
        : m_pass( p )
    {}
...
bool test_type( util::logger &log, queue & sycl_queue )
{
    ...
    {
        pass_t acc_pass = buf_pass. template get_access<cl::sycl::access::mode::read_write, cl::sycl::access::target::global_buffer>( cgh );
        KERNEL kernel( acc_pass );
        cgh.single_task<KERNEL>( kernel );
    } );

    if ( pass == 0 )
        FAIL( log, "Data type error for vector" );
