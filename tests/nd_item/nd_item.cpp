...
using namespace cl::sycl;
    ...
    {
    protected:
        typedef accessor<int, dimensions, access::read, access::global_buffer> t_readAccess;
        typedef accessor<int, dimensions, access::write, access::global_buffer> t_writeAccess;
    ...
    public:
...

        {
            ...
            for (int i = 0; i < dimensions; ++i)
            {
                if (m_globalID[(int)globalIndex] != globalIndex)
                    failed = true;
            }
            ...
            m_o[int(globalIndex)] = failed ? 0 : 1;
        }
    };
    ...
    void test_item(util::logger & log, cl::sycl::queue &queue)
    {
        ...
        /* test 1 Dimension*/
        {
            ...
            command_group(queue, [&]()
            {
                auto accG = bufGlob.template get_access<access::read>();
                auto accL = bufLoc.template get_access<access::read>();
                auto accOut = bufOut.template get_access<access::write>();

                kernel_nd_item<1> kernel_1d(accG,accL, accOut);
                parallel_for(dataRange, kernel_1d);
        ...
        memset(dataOut.get(), 0, nSize * sizeof(int));
        {
            ...
            command_group(queue, [&]()
            {
                ...
                parallel_for(dataRange, kernel_2d);
            });
        ...
        memset(dataOut.get(), 0, nSize * sizeof(int));
        {
            command_group(queue, [&]()
            {
                ...
                parallel_for(dataRange, kernel_3d);
            });
        }
        /* check no errors are returned */
