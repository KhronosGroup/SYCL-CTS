...
void test_barrier(util::logger & log, cl::sycl::queue &queue)
{
    ...
    /* run kernel to swap adjancent work item's global & local ids*/
    {
        ...
        {
            auto accGlobal = globalBuf.get_access<cl::sycl::access::mode::read_write>( cgh );
            auto accScratchGlobal = scratchBuf.get_access<cl::sycl::access::mode::read_write>( cgh );
            auto accLocal = localBuf.get_access<cl::sycl::access::mode::read_write>( cgh );
            accessor<int, 1, cl::sycl::access::mode::read_write, cl::sycl::access::target::local> accScratchLocal( localRange, cgh );

            cgh.parallel_for<class combined_barrier_kernel>(NDRange, [=]( nd_item<1> item )
            {
