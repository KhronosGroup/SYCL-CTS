...
bool reduce_over_group_verify_helper(std::vector<InputT>& v_input,
                                     std::vector<OutputT>& v_output,
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    const size_t group_reduced = get_reduce_reference<with_init, OpT>(
        v_input_begin, v_input_begin + cur_local_size);
...
template <bool with_init, typename OpT, typename InputT, typename OutputT>
bool reduce_over_group_verifier(std::vector<InputT>& v_input,
                                std::vector<OutputT>& v_output,
                                size_t global_size, size_t local_size) {
...
template <bool with_init, typename OpT, typename InputT, typename OutputT>
bool reduce_over_group_sg_verifier(std::vector<InputT>& v_input,
                                   std::vector<OutputT>& v_output,
                                   size_t global_size, size_t local_size,
                                   size_t sg_size) {
  bool res = false;
...
class invoke_init_joint_reduce_group {
 ...
 public:
  void operator()(sycl::queue& queue, const std::string& op_name) {
    if constexpr (std::is_same_v<std::remove_cv_t<RetT>, sycl::half>) {
      if (!queue.get_device().has(sycl::aspect::fp16))
        SKIP(
            "Device does not support half precision floating point "
            "operations.");
    } else if (std::is_same_v<std::remove_cv_t<RetT>, double>) {
      if (!queue.get_device().has(sycl::aspect::fp64))
        SKIP(
            "Device does not support double precision floating point "
            "operations.");
...
void reduce_over_group(sycl::queue& queue, const std::string& op_name) {
  ...
  size_t sg_size = 0;
  {
    ...
    queue.submit([&](sycl::handler& cgh) {
      ...
      auto g_output_acc =
          g_output_sycl.template get_access<sycl::access::mode::read_write>(
              cgh);
      auto sg_output_acc =
          sg_output_sycl.template get_access<sycl::access::mode::read_write>(
              cgh);
      ...
      cgh.parallel_for<reduce_over_group_kernel<D, T, OpT>>(
          executionRange, [=](sycl::nd_item<D> item) {
            ...
            ASSERT_RETURN_TYPE(
                T, sycl::reduce_over_group(group, v_acc[index], OpT()),
  ...
  // Verify return value for reduce_over_group on sub_group
  {
    res = reduce_over_group_sg_verifier<false, OpT>(
        v, sg_output, work_group_size, work_group_size, sg_size);
    std::string work_group = sycl_cts::util::work_group_print(work_group_range);
    CAPTURE(D, work_group);
    INFO("Value of " << test_names[0] << " with " << op_name
    ...
    queue.submit([&](sycl::handler& cgh) {
      ...
      cgh.parallel_for<init_reduce_over_group_kernel<D, T, U, OpT>>(
          executionRange, [=](sycl::nd_item<D> item) {
            ...
            ASSERT_RETURN_TYPE(
                T, sycl::reduce_over_group(group, v_acc[index], T(init), OpT()),
            ...
            ASSERT_RETURN_TYPE(
                ...
                "BinaryOperation binary_op) is wrong\n");
            sg_output_acc[index] = sycl::reduce_over_group(
                sub_group, v_acc[index], T(init), OpT());
          });
    });
  }
