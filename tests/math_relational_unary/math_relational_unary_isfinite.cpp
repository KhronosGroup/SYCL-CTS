...
#define TEST_NAME    math_relational_unary_isfinite
#define TEST_TYPE    isfinite
#define KERNEL_NAME  cKernel_isfinite

namespace math_relational_unary_isfinite__
{
...
{
protected:
    typedef accessor<T_in , 1, cl::sycl::access::mode::read , cl::sycl::access::target::global_buffer> t_readAccess;
    typedef accessor<T_out, 1, cl::sycl::access::mode::write, cl::sycl::access::target::global_buffer> t_writeAccess;
    ...
    bool execute( util::logger &log )
    {
        ...
        try
        {
            ...
            l_queue.submit( [&]( handler& cgh )
            {
                ...
                /* execute the kernel */
                cgh.parallel_for( nd_range<1>( range<1>( nBufferSize ), range<1>( nBufferSize / 8 ) ), kern );
            } );
...
} /* namespace math_relational_unary_isfinite_$NUM_ELMS$__ */
