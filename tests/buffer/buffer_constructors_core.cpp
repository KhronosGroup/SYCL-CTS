...
#ifdef UINT8_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::uint8_t>(
        log, "std::uint8_t");
#endif
#ifdef INT16_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::int16_t>(
        log, "std::int16_t");
#endif
#ifdef UINT16_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::uint16_t>(
        log, "std::uint16_t");
#endif
#ifdef INT32_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::int32_t>(
        log, "std::int32_t");
#endif
#ifdef UINT32_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::uint32_t>(
        log, "std::uint32_t");
#endif
#ifdef INT64_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::int64_t>(
        log, "std::int64_t");
#endif
#ifdef UINT64_MAX
    for_type_and_vectors<
        buffer_constructors_common::check_buffer_ctors_for_type, std::uint64_t>(
        log, "std::uint64_t");
#endif
#else
    for_all_types_and_vectors<
