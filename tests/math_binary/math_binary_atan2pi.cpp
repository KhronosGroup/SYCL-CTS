...
/** test specifiers
 */
#define TEST_NAME math_binary_atan2pi
#define WIMPY_MODE 1
#define MAX_ULPS 6.0f
#define HOST_FUNC atan2pi
#define REF_FUNC reference_atan2pi

namespace math_unary_atan2pi__ {
...
class test_kernel {
 public:
  ...
  test_kernel(t_write w_, t_read p1_, t_read p2_)
      : m_out(w_), m_p1(p1_), m_p2(p2_) {}
...
type_t type_get(vec<type_t, length> &ref, const uint32_t index) {
  assert(index < length);
  return ::getElement<type_t, length>(ref, index);
}
...
class test_class {
 protected:
  ...
  bool execute_chunk(util::logger &log, queue &a_queue,
    ...
    try {
      ...
      a_queue.submit([&](handler &cgh) {
        ...
        auto kern = test_kernel<type_t>(acc_output, acc_p1, acc_p2);
  ...
  bool verify_scalar(util::logger &log, base_t vut, base_t p1, base_t p2) {
    double ref = REF_FUNC((double)p1, (double)p2);
  ...
  bool verify(util::logger &log) {
    ...
    for (uint32_t i = 0; i < nBufferSize; i++) {
      for (uint32_t j = 0; j < num_elms; j++) {
        ...
        if (!verify_scalar(log, res, p1, p2)) return false;
      }
    }
