...
class test_functor_1 {
  sycl::accessor<float, 1, sycl::access_mode::read_write, sycl::target::device>
      m_acc;

 public:
  test_functor_1(sycl::accessor<float, 1, sycl::access_mode::read_write,
                                sycl::target::device>
                     acc)
      : m_acc(acc) {}
  ...
  void run(util::logger &log) override {
    ...
    log.note("check equality operator");
    {
      ...
      if (!(programA == programC) &&
        ...
        FAIL(log, "program equality does not work correctly. (copy assigned)");
      }
#endif
      if (programA != programB) {
        FAIL(log,
