  ...
  void run(util::logger &log) override {
...

    {
      ...
      try {
        ...
        // Check get_binaries()
        std::vector<std::vector<char>> binaries = prog.get_binaries();

        // Check get_context()
        sycl::context progCtx = prog.get_context();

        // Check get_compile_options()
        std::string progCompileOptions = prog.get_compile_options();
...

    {
      ...
      try {
        ...
        myQueue.submit(
            [&](sycl::handler &cgh) { cgh.single_task(program_kernel<0>()); });
...

    {
      ...
      try {
        ...
        if (prog.get_build_options().find(linkOptions) == std::string::npos) {
        ...
        myQueue.submit(
            [&](sycl::handler &cgh) { cgh.single_task(program_kernel<1>()); });
        myQueue.wait_and_throw();
...

    {
      ...
      try {
        ...
        myQueue.submit(
            [&](sycl::handler &cgh) { cgh.single_task(program_kernel<2>()); });
        myQueue.wait_and_throw();
...

    {
      ...
      try {
        ...
        myQueue.submit(
            [&](sycl::handler &cgh) { cgh.single_task(program_kernel<3>()); });
        myQueue.wait_and_throw();
