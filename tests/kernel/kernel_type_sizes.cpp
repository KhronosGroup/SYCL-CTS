...
{
    typedef accessor<int32_t, 1, access::write, access::global_buffer> write_t;

    write_t m_out;

public:
    type_size_kernel( write_t out_accessor )
        : m_out( out_accessor )
    {
    }
...
template <typename T>
bool test_kernel_type_size( util::logger &log, queue & sycl_queue, const sycl_cts::util::STRING &name )
{
...
#if 0
    printf( "%s: host-%d, device-%d\n", name.c_str(), sizeof( T ), kernel_type_size );
#endif
...
public:
    ...
    virtual void run( util::logger &log )
    {
        try
        {
            ...
            if ( !pass )
                FAIL( log, "one or more type size mismatches" );
