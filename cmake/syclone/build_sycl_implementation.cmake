...
# BUILD_SYCL IMPLEMENTATION
# Builds a SYCL test executable, compiling multiple SYCL test case source files, invoking a single-source/device compiler
# Parameters are:
...
#   - destination_path  Path where the output files (exe, stub-files, etc) will be placed
#
function(BUILD_SYCL_IMPLEMENTATION exe_name test_main test_cases_list destination_path)
	set(source_files
	${test_main}
	${test_cases_list}
	)
	...
	# cannot use include_directories or all the includes will be appended
	set_property(TARGET ${exe_name} APPEND PROPERTY INCLUDE_DIRECTORIES
	${destination_path}
	...
	if ( MSVC )
		set_target_properties(${exe_name} PROPERTIES LIBRARY_OUTPUT_DIRECTORY ${destination_path})
		set_target_properties(${exe_tname} PROPERTIES LIBRARY_OUTPUT_DIRECTORY_DEBUG ${destination_path})
		set_target_properties(${exe_tname} PROPERTIES LIBRARY_OUTPUT_DIRECTORY_RELEASE ${destination_path})
	endif ( MSVC )
	...
	foreach(source_file ${test_cases_list})
		...
		if(${found} EQUAL -1)
			build_spir("${exe_name}" "${spir_target_name}" "${source_file}" "${destination_path}")
			list(APPEND spir_targets_list ${spir_target_name})
		endif()
		add_dependencies(${exe_name} ${spir_target_name}_spir)
		list(APPEND output_stubs_list ${source_file}.sycl)
	endforeach()	
	...
	if(WIN32)
		target_link_libraries(${exe_name} ${ALEX_LIB_NAME})
	endif(WIN32)
	...
	set_target_properties(${exe_name} PROPERTIES RUNTIME_OUTPUT_DIRECTORY ${destination_path}/)
	set_target_properties(${exe_name} PROPERTIES RUNTIME_OUTPUT_DIRECTORY_DEBUG ${destination_path}/)
	set_target_properties(${exe_name} PROPERTIES RUNTIME_OUTPUT_DIRECTORY_RELEASE ${destination_path}/)
