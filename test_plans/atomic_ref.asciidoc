...
Note: for atomic load/stores and other atomic functions they might be called in a `while` loop simillar to how it is done in [/tests/usm/usm_atomic_access.h](https://github.com/KhronosGroup/SYCL-CTS/blob/SYCL-2020/tests/usm/usm_atomic_access.h#L115).
